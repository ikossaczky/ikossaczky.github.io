---
layout: post
title: "From Likelihood to Bayesian estimation of parameter distribution"
date: 2024-01-29 18:00:00 +0100
categories: mathematics
---

*Bayes theorem provides a simple tool for estimating the probabilistic distribution of unknown data distribution parameters, based on measured data. However, this approach is often considered cumbersome, due to the need for a prior distribution of $$\theta$$. In this post, we  show how the need for prior distribution naturally emerges from trying to transform the likelihood function into the distribution of $$\theta$$ and discuss cases, when prior distribution is not needed.*
...
Look at equation \eqref{eq:cond_pdf2}: it is the standard equation for Bayesian parameter estimation, and we got it just by trying to get reasonable PDF $$p(\theta \vert y)$$ from a probabilistic model $$p(y \vert \theta)$$ and a known measurement of $$y$$. Let's recapitulate the Bayesian interpretation of the expressions composing equation \eqref{eq:cond_pdf2}:

- $$p(y \vert \theta)$$ is a probability of $$y$$ for given $$\theta$$. As a function of $$\theta$$, it is likelihood of $$\theta$$, given some measured $$y$$.
...
In Bayesian view, this scenario is equivalent to probability distribution with a constant probability density over the whole definition domain of $$\theta$$. It means, we consider every $$\theta$$ as equally likely, or we do not have any prior belief about the distribution of $$\theta$$ at all. Indeed, if we substitute a constant prior density
...
Moreover, we can consider our dataset of measurements $$y_0, y_1, \dots y_N$$ as a single measurement of the random vector $$Y$$, and all the derivations in the previous section are still applicable.
